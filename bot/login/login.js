...
async function startBot(loginWithEmail) {
        ...
        (function loginBot(appState) {
                ...
                login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
                        ...
                        // ——————————————————— DASHBOARD ——————————————————— //
                        // IPC: report live stats up to the supervisor (start.js) so the parent
// dashboard's /api/stats stays accurate. Safe no-op when run standalone.
if (process.send && !global._statsReporterInstalled) {
        global._statsReporterInstalled = true;
        const sendStats = () => {
                try {
                        process.send({
                                type: "stats",
                                data: {
                                        ...
                                        aliases: global.GoatBot.aliases?.size || 0,
                                        eventCommands: global.GoatBot.eventCommands?.size || 0,
